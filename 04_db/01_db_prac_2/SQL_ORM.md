...
2. user 레코드 생성
   ...
   ```python
   # orm
   User.objects.create(first_name='도훈',last_name='홍',age='27',country='대구',phone='010-4513-6506',balance='999999')
   <User: User object (102)>
   ```

   ```sql
   -- sql
   INSERT INTO users_user VALUES ('101','도훈','김','27','대구','010-4513-6506','99999999');
   ```
...
```
      ...
      ```sql
   -- sql
   SELECT first_name FROM users_user WHERE (last_name LIKE '황%') AND ( country LIKE '강원도')
...
1. 나이가 많은 사람순으로 10명
   ...
   ```
      ...
      ```sql
   -- sql
   SELECT first_name FROM users_user ORDER BY age DESC LIMiT 10;
      ```
...
2. 잔액이 적은 사람순으로 10명
   ...
   ```
      ...
      ```sql
   -- sql
   SELECT first_name FROM users_user ORDER BY balance LIMIT 10;
      ```
...
```
   ...
   ```sql
   -- sql
   SELECT first_name FROM users_user ORDER BY balance ASC, age DESC LIMIT 10;
   ```
   
4. 성, 이름 내림차순 순으로 5번째 있는 사람s
